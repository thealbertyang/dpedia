const reducerName = "REVIEWS_CATEGORIES";

export default function reviewsCategoriesReducer(state = {
	busy: null,
	delete: null,
	pendingCreate: null,
	pendingUpdate: null,
	...
	switch(action.type) {
		case reducerName+"_SEARCH": {
			state = {...state, status: 'searching_resource' }
			break;
		}
		case reducerName+"_SEARCH_SUCCESS": {
			console.log('success search')
			state = {...state, status: 'search_resources_success', records: action.payload, results: {...state.results, to: action.to, from: action.from, total: action.total, current: action.current, prev_url: action.prev_url, next_url: action.next_url} }
			break;
		}
		case reducerName+"_SEARCH_ERROR": {
			console.log('what', reducerName);
			state = {...state, status: 'search_resource_error' }
			break;
		}		
