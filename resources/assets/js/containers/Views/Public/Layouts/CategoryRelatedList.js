...
export class ResourceRelatedList extends React.Component {
	constructor(props){
		super(props)
		this.state = { count: 8 };
	}

	filterCategories = (records, category_id) => {
		records.filter((item)=>{
			if(item.reviews_category_id == this.props.reviews.data.reviews_category_id && item.status == 'published'){
				return item;
			}
		})
	}
