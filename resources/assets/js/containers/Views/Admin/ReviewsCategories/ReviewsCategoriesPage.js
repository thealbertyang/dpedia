...
@connect((store) => {
	return {
		...
		results: store.reviews_categories.results,
	}
})
export class ReviewsCategoriesPage extends React.Component {
	constructor(props){
		super(props);	
	...
	render(){
		return (
			<IndexCreateEditContainer type="INDEX" title="Reviews Categories" matchUrl={this.props.match.url}>
	                ...
	                {this.props.records && 
		                ...
		                (
							<tr key={data.id}>
				                  ...
				                  <td><a href="#" onClick={(e) => { e.preventDefault(); this.props.dispatch(resourceCrudActions.delete(data.id)) }}><i className="fa fa-trash"></i></a> <Link to={`${this.props.match.url}/${data.id}/edit`}><i className="fa fa-pencil-alt"></i></Link></td>
							</tr>
						)) 
