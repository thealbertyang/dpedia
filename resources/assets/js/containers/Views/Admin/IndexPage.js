...
export class IndexPage extends React.Component {
	...
	componentWillMount(){
		...
		this.props.dispatch(reviewsActions.crudGetAll(pageNumber));   
		} 
	...
	componentDidUpdate(prevProps, prevState){
		...
		//if next page was called then we want to update reviews list, when it does update, we don't want it to update again if it's the same page now
		if(pageNumberNext !== pageNumber){
			this.props.dispatch(reviewsActions.crudGetAll(pageNumberNext));
		}
	}
	...
	deleteUser(e, id){
		e.preventDefault();
		this.props.dispatch(reviewsActions.crudDelete(id));
	}

	render(){
		return (
			<IndexCreateEditContainer type="INDEX" title="Reviews" matchUrl={this.props.match.url}>
	                ...
	                {this.props.records && 
		                ...
		                (
							<tr key={data.id}>
				                  ...
				                  <td><a href="#" onClick={(e) => { e.preventDefault(); this.props.dispatch(reviewsActions.crudDelete(data.id)) }}><i className="fa fa-trash"></i></a> <Link to={`${this.props.match.url}/${data.id}/edit`}><i className="fa fa-pencil-alt"></i></Link></td>
							</tr>
						)) 
