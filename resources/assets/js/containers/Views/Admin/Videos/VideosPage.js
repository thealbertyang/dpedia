...
@connect((store) => {
	return {
		...
		records: store.videos.records,
		status: store.videos.status,
		errors: store.videos.errors,
		results: store.videos.results,
	}
})
export class VideosPage extends React.Component {
	...
	componentWillMount(){
		...
		this.props.dispatch(videosActions.crudGetAll(pageNumber));   
		} 
	...
	componentDidUpdate(prevProps, prevState){
		...
		if(pageNumberNext !== pageNumber){
			this.props.dispatch(videosActions.crudGetAll(pageNumberNext));
		}
	}
	                ...
	                {this.props.records && 
		                ...
		                (
							<tr key={data.id}>
				                  ...
				                  <td><a href="#" onClick={(e) => { e.preventDefault(); this.props.dispatch(videosActions.crudDelete(data.id)) }}><i className="fa fa-trash"></i></a> <Link to={`${this.props.match.url}/${data.id}/edit`}><i className="fa fa-pencil-alt"></i></Link></td>
							</tr>
						)) 
